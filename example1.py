...
mean = np.load("./examples/img_mean.npy").transpose((1, 2, 0)) # load mean image of imagenet dataset
...
X = tf.placeholder(tf.float32, shape = [None, 224, 224, 3],name="zlong_X") # placeholder for input images
...
y_conv_1 = tf.nn.conv2d(X, filter=W_conv_1, strides=[1, 4, 4, 1], padding="SAME",name="zlong_conv_1") + b_conv_1
...
im = np.expand_dims(imresize(imresize(imread('data/1.jpg'), (256, 256)) - mean,
                             (224, 224)), axis = 0)
